import { desktopHandleToggleFullScreen, desktopHandleClose, desktopQuitApp } from '@/utils/desktop-stubs';
...
const useBookShortcuts = ({ sideBarBookKey, bookKeys }: UseBookShortcutsProps) => {
  ...
  const toggleFullscreen = async () => {
    if (isDesktopAppPlatform()) {
      await desktopHandleToggleFullScreen();
    }
  };

  const closeWindow = async () => {
    if (isDesktopAppPlatform()) {
      await desktopHandleClose();
    }
  };
