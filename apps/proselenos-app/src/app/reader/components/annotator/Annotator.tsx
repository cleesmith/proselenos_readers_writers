...
const Annotator: React.FC<{ bookKey: string }> = ({ bookKey }) => {
  ...
  useEffect(() => {
    handleShowPopup(showAnnotPopup || showWiktionaryPopup || showWikipediaPopup);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showAnnotPopup, showWiktionaryPopup, showWikipediaPopup]);
  ...
  const handleExportMarkdown = (event: CustomEvent) => {
    ...
    lines.push('');
    lines.push(`**${_('Exported from Proselenosebooks')}**: ${new Date().toISOString().slice(0, 10)}`);
    lines.push('');
    lines.push('---');
    lines.push('');
