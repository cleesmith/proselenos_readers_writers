...
import { desktopHandleClose, desktopHandleOnCloseWindow, parseOpenWithFiles } from '@/utils/desktop-stubs';
...
import { getCurrentWindow, UnlistenFn } from '@/utils/desktop-stubs';
...
const ReaderContent: React.FC<{ ids?: string; settings: SystemSettings; onClose?: () => void }> = ({ ids, settings, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (isDesktopAppPlatform()) {
      unlistenOnCloseWindow = desktopHandleOnCloseWindow(handleCloseBooks);
    }
    ...
    if (bookKeys.filter((key) => key !== bookKey).length == 0) {
      ...
      if (appService?.hasWindow) {
        if (openWithFiles.length > 0) {
          return await desktopHandleClose();
        }
        const currentWindow = getCurrentWindow();
