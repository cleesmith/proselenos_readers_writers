...
export const Toast = () => {
  ...
  const toastClassMap = {
    info: 'toast-info toast-top sm:toast-end toast-center',
  ...
  useEffect(() => {
    ...
    // If timeout is 0, persist indefinitely (no auto-dismiss)
    if (toastTimeout === 0) {
      return;
    }
  ...
  const handleShowToast = async (event: CustomEvent) => {
    ...
    setToastType(type);
    // If timeout is specified, use it; otherwise reset to default 5000ms
    setToastTimeout(timeout !== undefined ? timeout : 5000);
