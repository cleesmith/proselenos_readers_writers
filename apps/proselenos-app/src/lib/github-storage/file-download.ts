...
/**
 * Download a binary file from GitHub repo (returns Buffer instead of ArrayBuffer)
 * Use this for binary files like .docx, .epub, etc. to avoid conversion issues
 */
export async function downloadBinaryFile(
  ...
  try {
    ...
    // Decode base64 directly to Buffer (no ArrayBuffer conversion)
    const buffer = Buffer.from(blob.content, 'base64');
    ...
    console.error('Failed to download binary file from repo:', error);
    throw error;
  }
}
